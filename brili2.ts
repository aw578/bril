...
function evalInstr(instr: bril.Instruction, state: State): Action {
  if (instructionsEvaluated < 1000) {
    if (instr.op === "br") {
      const cond = getBool(instr as bril.EffectOperation, state.env, 0);
      console.log(JSON.stringify({ ...instr, branch: cond ? 0 : 1 }));
    }
    else {
...
function evalFunc(func: bril.Function, state: State): Value | null {
  for (let i = 0; i < func.instrs.length; ++i) {
    ...
    else if (instructionsEvaluated < 1000) {
