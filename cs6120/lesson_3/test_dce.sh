...
for f in benchmarks/core/*.bril; do
    ...
    total_inst2=$(bril2json < "$f" | python cs6120/lesson_3/dce.py | brili -p $args 2>&1 > /dev/null | grep -oP '(?<=total_dyn_inst: )\d+')    
    if [ "$total_inst1" -lt "$total_inst2" ]; then
        echo "FAIL (ins) $f $total_inst1 $total_inst2"
    fi    
    # output comparison
    output1=$(bril2json < "$f" | brili -p $args 2>/dev/null)    
    output2=$(bril2json < "$f" | python cs6120/lesson_3/dce.py | brili -p $args 2>/dev/null)  
